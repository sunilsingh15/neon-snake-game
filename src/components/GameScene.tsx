...
import { Sphere, Grid, Trail, Sparkles } from '@react-three/drei';
...
function BoostTrail({ playerId, color }: { playerId: string, color: string }) {
  ...
  const headTrailRef = useRef<THREE.Mesh>(null);
  ...
  useFrame(() => {
    ...
    if (!player || !player.isBoosting || player.segments.length === 0) {
      if (afterImageRef.current) afterImageRef.current.count = 0;
      if (headTrailRef.current) headTrailRef.current.visible = false;
      if (sparklesGroup.current) sparklesGroup.current.visible = false;
      return;
    }

    if (sparklesGroup.current) sparklesGroup.current.visible = true;
    if (headTrailRef.current) headTrailRef.current.visible = true;

    const head = player.segments[0];
    if (sparklesGroup.current) {
      sparklesGroup.current.position.set(head.x, head.y, 0.5);
    }
    
    if (headTrailRef.current) {
      headTrailRef.current.position.set(head.x, head.y, 0.5);
    }

    if (afterImageRef.current) {
      const count = player.segments.length;
      afterImageRef.current.count = count;
      for (let i = 0; i < count; i++) {
        const seg = player.segments[i];
        dummy.position.set(seg.x, seg.y, 0.45);
        // Fade and shrink after-images slightly
        const scale = 0.7 + Math.sin(Date.now() * 0.01 + i) * 0.1;
        dummy.scale.setScalar(scale);
        dummy.updateMatrix();
        afterImageRef.current.setMatrixAt(i, dummy.matrix);
      }
      afterImageRef.current.instanceMatrix.needsUpdate = true;
    }
  });
  ...
  return (
    <group>
      ...
      <instancedMesh ref={afterImageRef} args={[null as any, null as any, 1000]} frustumCulled={false}>
...
function Snake({ playerId, color, isLocal }: { playerId: string, color: string, isLocal: boolean }) {
  ...
  return (
    <group>
      {isLocal && <BoostTrail playerId={playerId} color={color} />}
